// Imports from Next.js and project-specific modules
import Link from "next/link";
import Image from "next/image";
import { getServerSession } from "next-auth/next";
import { redirect, notFound } from "next/navigation";

import { Ad } from "@/lib/db/models"; // Mongoose model for ads
import { connectToMongoDB } from "@/lib/db/mongodb"; // MongoDB connection helper

import { authOptions } from "@/lib/auth"; // Authentication config

/**
 * Fetches an ad by its ID from the MongoDB database.
 * Populates related user, category, and subcategory fields.
 * Converts the result to a plain object using `.lean()`.
 */
async function getAd(id: string) {
  try {
    ...
    // Serialize the document to ensure it's JSON-safe
...
/**
 * Renders the ad detail page based on the given ad ID from the route parameters.
 * Only owners and moderators can see pending or rejected ads.
 */
export default async function AdDetailPage({
  ...
  // If the ad doesn't exist, show 404 page
  if (!ad) {
    notFound();
  }

  // Only allow owners or moderators to view non-approved ads
  if (
    ad.status === "pending" &&
    (!session?.user ||
        ...
        >
          {ad.status === "pending" ? (
            ...
            <>
              ...
              {/* Edit option for owner of rejected ad */}
              {isOwner && (
                <div className="mt-4">
                  <Link
                    href={`/ads/${id}/edit`}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="p-4 lg:p-6 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-700">
            ...
            {/* Moderation options for moderators */}
